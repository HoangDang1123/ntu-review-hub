...
export default function UnitScreen() {
  ...
  useEffect(() => {
    setActiveModal(/^\/admin\/unit\//.test(location.pathname)); // Lấy tất cả đường dẫn sau
  }, [location.pathname]);
  ...
  const handleDelete = async (id) => {
    try {
      realtimeDB.ref("units").child(id).remove();
  ...
  const handleOpenAddForm = async () => {
    navigate("/admin/unit/add-unit");
  };

  const handleOpenEditForm = async (id) => {
    navigate(`/admin/unit/edit-unit/${id}`);
  }

  return (
    <div style={{ position: 'relative' }}>
      <div className={`page-container item ${activeModal ? "activeForm" : ""}`}>
        <label className="page-label">Đơn vị công tác</label>

        <div className="title-container">
          <label className="title-label">Danh sách đơn vị công tác</label>
        ...
        <table className="table-container">
          <thead>
            <tr>
              <th className="num">#</th>
              <th>Tên đơn vị</th>
              <th>Tên trưởng đơn vị</th>
              <th>Mã số trưởng đơn vị</th>
              <th></th>
            </tr>
          </thead>
          <tbody>
